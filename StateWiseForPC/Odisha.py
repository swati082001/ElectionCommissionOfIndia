...
if firstPageReq.status_code == 200:
    ...
    state_option = dropdown.find('option', string='Odisha')
    if state_option:
        state_value = state_option['value']
    # Construct the URL for fetching results for Odisha
        ...
        print("No state option as Odisha found")   
else:
...
merged_df.to_csv('CSV Files/Odisha_PC_electionresults.csv', index=False)
