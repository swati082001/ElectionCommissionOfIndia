...
if firstPageReq.status_code == 200:
    ...
    state_option = dropdown.find('option', string='Madhya Pradesh')
    if state_option:
        state_value = state_option['value']
    # Construct the URL for fetching results for Madhya Pradesh
        ...
        print("No state option as Madhya Pradesh found")   
else:
...
merged_df.to_csv('CSV Files/Madhya Pradesh_PC_electionresults.csv', index=False)
