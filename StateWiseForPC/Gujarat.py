...
if firstPageReq.status_code == 200:
    ...
    state_option = dropdown.find('option', string='Gujarat')
    if state_option:
        state_value = state_option['value']
    # Construct the URL for fetching results for Gujarat
        ...
        print("No state option as Gujarat found")   
else:
...
merged_df.to_csv('CSV Files/Gujarat_PC_electionresults.csv', index=False)
