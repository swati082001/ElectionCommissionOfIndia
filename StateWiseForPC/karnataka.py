...
if firstPageReq.status_code == 200:
    ...
    state_option = dropdown.find('option', string='Karnataka')
    if state_option:
        state_value = state_option['value']
    # Construct the URL for fetching results for Karnataka
        ...
        print("No state option as Karnataka found")   
else:
...
merged_df.to_csv('CSV Files/Karnataka_PC_electionresults.csv', index=False)
